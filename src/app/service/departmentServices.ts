...
export class DepartmentService {
  ...
  getDeptById(deptId: number): Observable<any> {
    return this.http
      .get<any>(BASE_API_URL + `/dept/get/` + deptId, {
      ...
      .pipe(
        ...
        catchError((error) => {
          ...
          return throwError(
            'Something went wrong while fetching department data.'
          );
        })
      );
  }
  ...
  addDepartment(dept: any) {
    ...
    return this.http.post<any>(BASE_API_URL + `/dept/add`, dept, {
      headers: this.getHeaders(),
    });
  }
  ...
  deleteDepartment(deptId: number) {
    return this.http.post<String>(
      BASE_API_URL + `/dept/delete/` + deptId,
      deptId,
      { headers: this.getHeaders() }
    );
  }
  ...
  updateDepartment(deptId: number, dept: any) {
    return this.http.post<any>(BASE_API_URL + `/dept/update/` + deptId, dept, {
      headers: this.getHeaders(),
    });
  }
  ...
  getAllDDept() {
    return this.http.get<any>(
      BASE_API_URL +
        `/dept/getAllDepartments?pgn=&sz=2000` 
        ,
      { headers: this.getHeaders() }
    );
  }
